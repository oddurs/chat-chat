...
import { configNames, modelStats, pairStats, type PairStat } from "@/lib/stats";
...
export default async function Models({ searchParams }: PageProps<"/models">) {
  ...
  const scope = typeof p?.config === "string" ? p.config : CONTROLLED;
  const controlled = scope === CONTROLLED;
  const config = scope === "all" ? undefined : scope;

  const stats = modelStats(config);
  const pairs = pairStats(config);
    ...
    <div className="flex flex-col gap-7">
      <header className="flex flex-col gap-2">
        ...
        <nav className="flex flex-wrap gap-2 text-[12px]">
          {[CONTROLLED, "all", ...configs.filter((c) => c !== CONTROLLED)].map((name) => (
            <Link
              key={name}
              href={`/models?config=${name}`}
      ...
      {pairs.length === 0 ? (
        <p className="rounded-xl border border-line bg-panel p-6 text-[14px] text-muted">
          No runs for this scope yet. The tournament is{" "}
          <code className="font-mono text-accent">./chatchat.py batch matrix-tournament.toml</code>.
        </p>
      ) : (
        <>
